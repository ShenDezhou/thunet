...
class MultiHeadedAttentionModule(ModuleBase):
    ...
    def backward(self, dLdy):
        ...
        if(type(dLdQ_proj) == list):
            dLdQ_proj = dLdQ_proj[0]
        if (type(dLdK_proj) == list):
            dLdK_proj = dLdK_proj[0]
        if (type(dLdV_proj) == list):
            dLdV_proj = dLdV_proj[0]
...
class MultiHeadedAttentionModule_new(ModuleBase):
    def __init__(self, n_heads=8, dropout_p=0, init="glorot_uniform", optimizer=None):
        """
        A mutli-headed attention module.
    ...
    def _init_params(self):
        ...
        self.attention = DotProductAttention_new(scale=True, dropout_p=self.dropout_p)
        self.projections = {
            k: Dropout(
                FullyConnected(
                    init=self.init,
